...
def build_df():
    ...
    for c in chunks:
        ...
        for n in negative:
            ...
            df = {'text' : text, 'sentiment-p' : TextBlob(text).sentiment.polarity, 'sentiment-s' : TextBlob(text).sentiment.subjectivity}
    ...
    result_neg = preprocess(result_neg)
    print(result_neg)
    print("result_neg sentiment-p: " + str(result_neg['sentiment-p'].mean()))
    print("result_neg sentiment-s: " + str(result_neg['sentiment-s'].mean()))
    common = list((pd.Series(' '.join(result_neg['text']).split()).value_counts()[:10]).index)
    print(common)
    ...
    result_pos = preprocess(result_pos)
    print(result_pos)
    print("result_pos sentiment-p: " + str(result_pos['sentiment-p'].mean()))
    print("result_pos sentiment-s: " + str(result_pos['sentiment-s'].mean()))
    common = list((pd.Series(' '.join(result_pos['text']).split()).value_counts()[:10]).index)
    print(common)
...
blob1 = TextBlob("took find antidepressants worked first type")
print(format(blob1.sentiment)) # Sentiment(polarity=-0.8, subjectivity=0.9)
